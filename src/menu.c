...
static int search_root() {
	...
	while ((ent = fs_readdir(hnd)) != NULL) {
		...
		if(!search_root_check(ent->name, "", "/restoreb.ldr")) {

			snprintf(item->path, MAX_FN_LEN, "/%s/restoreb.ldr", ent->name);

		} else if (!search_root_check(ent->name, "/boot", "/ecsb.ldr")) {

			snprintf(item->path, MAX_FN_LEN, "/%s/boot/ecsb.ldr", ent->name);
